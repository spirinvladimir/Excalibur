...
module ex {
   ...
   export class Engine extends ex.Class {
      ...
      */
      ...
      private drawPolarLoadingBar(ctx: CanvasRenderingContext2D, loaded: number, total: number) {
         if (this.loadingDraw) {
            this.loadingDraw(ctx, loaded, total);
            return;
         }
         ...
         var oldAntialias = this.getAntialiasing();
         this.setAntialiasing(true);
         ctx.drawImage(image, 0, 0, 800, 300, this.getWidth()/2 - 45, this.getHeight()/2 - 45, width, imageHeight);
         ctx.fillStyle = 'black';
         ctx.fillRect(this.getWidth()/2+37, this.getHeight()/2 -15, 800, 300);

         
         ctx.beginPath();
         ctx.arc(this.getWidth()/2, this.getHeight()/2, 74, 0, Math.PI * 2 , false);
         ctx.arc(this.getWidth()/2, this.getHeight()/2, 96, Math.PI * 2 , 0, true);
         ctx.fillStyle = 'white';
         ctx.closePath();
         ctx.fill();

         ctx.beginPath();
         ctx.arc(this.getWidth()/2, this.getHeight()/2, 78, 0, Math.PI * 2 , false);
         ctx.arc(this.getWidth()/2, this.getHeight()/2, 92, Math.PI * 2 , 0, true);
         ctx.fillStyle = 'black';
         ctx.closePath();
         ctx.fill();

         ctx.beginPath();
         ctx.arc(this.getWidth()/2, this.getHeight()/2, 85, 0, Math.PI * 2 * (loaded/total), false);
         ctx.arc(this.getWidth()/2, this.getHeight()/2, 85, Math.PI * 2 * (loaded/total), 0, true);
         ctx.strokeStyle = 'white';
         ctx.lineWidth = 6;
         ctx.closePath();
         ctx.stroke();
